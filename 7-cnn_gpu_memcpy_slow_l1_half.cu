...
for (int i=0; i<wt_width; i++){
  for (int j=0; j<wt_width; j++){
   for(int k=0; k<num_ch; k++){
	 ...
	 d_o[blockIdx.x*num_wt*height*width+blockIdx.y*width*height+(2*row+y)*width+(2*col+x)] +=__half2float(d_i[blockIdx.x*num_ch*ip_height*ip_height+k*ip_height*ip_height+(stride*(2*row+y)+i)*ip_height+(stride*(2*col+x)+j)])*__half2float(d_w[blockIdx.y*num_ch*wt_width*wt_width+k*wt_width*wt_width+(i*wt_width+j)]);
}
}
}
} 
