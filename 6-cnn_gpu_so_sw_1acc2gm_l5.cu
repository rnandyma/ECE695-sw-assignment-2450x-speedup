...
__syncthreads();   
int shared_idx =row*blockDim.x+col;
//__syncthreads();
//float prod = 0;
for (int i=0; i<wt_width; i++){
  for (int j=0; j<wt_width; j++){
...
//	prod += ip*wt;
	s_o[shared_idx] += ip*wt;
    	//__syncthreads();
}
}
}
if(s_o[shared_idx]>=0)
d_o[blockIdx.x*num_wt*blockDim.x*blockDim.y+(blockIdx.y)*blockDim.x*blockDim.y+row*blockDim.x+col] = s_o[shared_idx];
//__syncthreads();
if(s_w[0]!=0){
for (int i=0; i<wt_width; i++){
    for (int j=0; j<wt_width; j++){
