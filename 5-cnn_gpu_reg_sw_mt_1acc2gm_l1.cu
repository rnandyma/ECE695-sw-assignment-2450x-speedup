...
if(row*blockDim.x+col<C*R*S)
{
 //   for(int k=0; k<num_ch; k++){
 //    s_w[k*wt_width*wt_width+(row*blockDim.x+col)] = d_w[(blockIdx.y)*num_ch*wt_width*wt_width+k*wt_width*wt_width+(row*blockDim.x+col)];
s_w[(row*blockDim.x+col)] = d_w[(blockIdx.y)*num_ch*wt_width*wt_width+(row*blockDim.x+col)];
