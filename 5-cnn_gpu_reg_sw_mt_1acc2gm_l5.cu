...
if (row*width+col<R*S)
{
     ...
     s_w[k*wt_width*wt_width+(row*width+col)] = d_w[(blockIdx.y)*num_ch*wt_width*wt_width+k*wt_width*wt_width+(row*width+col)];
...
if(row*width+col<R*S){
     for(int k=0; k<num_ch; k++){
     s_w[k*wt_width*wt_width+(row*width+col)] = 0;
__syncthreads();
}
}
}
