...
int shared_idx = (2*row+y)*width+(2*col+x);
for (int i=0; i<wt_width; i++){
  for (int j=0; j<wt_width; j++){
   for(int k=0; k<num_ch; k++){
    float ip = d_i[blockIdx.x*num_ch*ip_height*ip_height+k*ip_height*ip_height+(stride*(row+y*blockDim.y)+i)*ip_height+(stride*(col+x*blockDim.x)+j)];
        //float wt1 = d_w[blockIdx.y*num_ch*wt_width*wt_width+k*wt_width*wt_width+(i*wt_width+j)];
//	wt = s_w[k*wt_width*wt_width+(i*wt_width+j)];
	if ((row+y*blockDim.y<height)&&(col+x*blockDim.x<width))
	s_o[shared_idx] += ip*s_w[k*wt_width*wt_width+(i*wt_width+j)];
	 __syncthreads();
//	prod += ip*wt;
	//d_o[blockIdx.x*num_wt*height*width+blockIdx.y*width*height+(2*row+y)*width+(2*col+x)]  = prod ;}
...
if((row+y*blockDim.y<height)&&(col+x*blockDim.x<width))
{if(s_o[shared_idx] >=0)
d_o[blockIdx.x*num_wt*height*width+blockIdx.y*width*height+(row+y*blockDim.y)*width+(col+x*blockDim.x)] =s_o[shared_idx] ;s_o[shared_idx] = 0;}
} 
}
if(s_w[0]!=0)
{
