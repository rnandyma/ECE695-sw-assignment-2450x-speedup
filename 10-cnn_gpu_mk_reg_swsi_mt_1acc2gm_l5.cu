...
if(s_i_idx+((H*W+1)/2) < H*W)
s_i[s_i_idx+((H*W+1)/2)]= d_i[blockIdx.x*num_ch*ip_height*ip_height+blockIdx.z*ip_height*ip_height+s_i_idx+((H*W+1)/2)];
}
__syncthreads();
//printf("ip_done\n");
float prod = 0;
...
for (int i=0; i<wt_width; i++){
  for (int j=0; j<wt_width; j++){
    float ip = s_i[(stride*row+i)*ip_height+(stride*col+j)];   
...
if(cudaSuccess != cudaMalloc((void**) &d_o,C*batch_size*M*E*F*sizeof(float)))
{
...
printf("cpu done\n");
//cpu_end = clock();
dim3 dimGrid(batch_size,256,384);
...
red_ch<<<dimGridRed, dimBlockRed>>>(d_r,d_o,384,batch_size,256);
//gpu_end = clock();
