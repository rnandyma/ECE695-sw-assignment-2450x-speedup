...
if((row<num_wt*height) && (col<num_img*width))
{
for (int i=0; i<wt_width; i++){
  for (int j=0; j<wt_width; j++){
   for(int k=0; k<num_ch; k++){
        d_o[blockIdx.x*num_wt*blockDim.x*blockDim.y+blockIdx.y*blockDim.x*blockDim.y+row*blockDim.x+col] += d_i[blockIdx.x*num_ch*ip_height*ip_height+k*ip_height*ip_height+(stride*(row)+i)*ip_height+(stride*(col)+j)]*d_w[blockIdx.y*num_ch*wt_width*wt_width+k*wt_width*wt_width+(i*wt_width+j)];
}
}
}
if(d_o[blockIdx.x*num_wt*blockDim.x*blockDim.y+blockIdx.y*blockDim.x*blockDim.y+row*blockDim.x+col]<0)
d_o[blockIdx.x*num_wt*blockDim.x*blockDim.y+blockIdx.y*blockDim.x*blockDim.y+row*blockDim.x+col]=0;
} 
}
...
//cudaMalloc((void**) &d_i,N*C*H*W*sizeof(float));
...
//gpu_start = clock();
ew_gpu_mmul<<<dimGrid, dimBlock>>>(OPG,IP,WT,13,13,1,15,3,256,batch_size,384);
