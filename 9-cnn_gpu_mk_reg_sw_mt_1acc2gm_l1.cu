...
{       //if((2*row+y<height)&&(2*col+x<width))
	red_sum += d_o[i*(num_wt*num_img*height*width)+blockIdx.x*num_wt*height*width+blockIdx.y*height*width+(2*row+y)*width+(2*col+x)] ;	
}
if((2*row+y<height)&&(2*col+x<width))
d_r[blockIdx.x*num_wt*height*width+blockIdx.y*height*width+(2*row+y)*width+(2*col+x)] = red_sum;
